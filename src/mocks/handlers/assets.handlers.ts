...
export const assetsHandlers = [
  http.get("/api/assets", ({ request }) => {
    ...
    if (fileName)  results = results.filter((a) => a.fileName.toLowerCase().includes(fileName));
    if (tags.length > 0) results = results.filter((a) => tags.some((t) => a.tags.includes(t)));
    if (uploadedAfter)  results = results.filter((a) => new Date(a.uploadedAt) >= new Date(uploadedAfter));
    if (uploadedBefore) results = results.filter((a) => new Date(a.uploadedAt) <= new Date(uploadedBefore));
  ...
  http.post("/api/assets", async ({ request }) => {
    ...
    const newAsset: Asset = {
      ...
      statusHistory: [
        { status: AssetStatus.PENDING_REVIEW, changedAt: new Date().toISOString() },
      ],
